...
base = 'https://cmse.buct.edu.cn/8340/list.htm'
headers = {
    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/105.0.0.0 Safari/537.36'}
root_url = "https://cmse.buct.edu.cn/8340/list.htm"
urls = UrlManager()
excelfile = xlsxwriter.Workbook("材料科学与工程/正高.xlsx")
...
def first():
    ...
    lists = soup.find("ul", 'subPhenomList subPhenomListS').find_all("a")
...
def craw(url, index):
    print("ready")
    print(url)
    if (url != "https://cmse.buct.edu.cn/8340/list.htm") &(url != "https://cmse.buct.edu.cn/2020/1102/c12598a147381/page.htm"):
        rd = requests.get(url=url, headers=headers)
        if rd.status_code == 200:
            rd.encoding = 'utf-8'
            soup = BeautifulSoup(rd.text, "html.parser")
            try:
                name = soup.find("div", class_='subLeaderDetail_info').find("h3").get_text()
                print(name)
            except:
                name = "error"
            #print(name)
            ...
            for i in img:
                ...
                if not (img is None or img.__sizeof__() < 10000):
                        print(image_url)
                    ...
                    """
            datarow = ['材料科学与工程/正高/'+name+'.jpg',name, content_]
                ...
                if re_url is not None:
                    ...
                    if  (re_url != "<img src=""/>"):
                        ...
                        if not (image is None ):
                                ...
                                with open('材料科学与工程/正高/{}.jpg'.format(name), 'wb') as f:
                                    f.write(image)
                                excel.write(index, 0, '材料科学与工程/正高' + name + '.jpg')
